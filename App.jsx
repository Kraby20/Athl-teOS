...
const DEFAULT_STATE = {
  settings: { theme: "dark", accent: "#FF5C38", name: "", pro: false },
  profile: {
...
export default function App() {
  ...
  const ctx = { state, update, t, accent, pro: !!state.settings.pro, setTab };
  ...
  return (
    <div style={{ background: t.bg, color: t.text, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      <header style={{ maxWidth: 480, margin: "0 auto", padding: "calc(env(safe-area-inset-top, 0px) + 14px) 18px 12px", display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        <div>
      ...
      <main style={{ padding: "0 14px 110px", maxWidth: 480, margin: "0 auto" }}>
        {tab === "muscu" && <Muscu {...ctx} />}
        {tab === "diete" && <Diete {...ctx} />}
        {tab === "suivi" && <Suivi {...ctx} />}
...
/* MUSCU                                                               */
/* ------------------------------------------------------------------ */
function Muscu({ state, update, t, accent, pro, setTab }) {
    ...
    <div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {state.programs.map((p) =>
          editing === p.id ? (
            <ProgramEditor key={p.id} {...{ state, update, t, accent, program: p, done: () => setEditing(null) }} />
          ) : (
...
/* DIÈTE                                                               */
/* ------------------------------------------------------------------ */
function Diete({ state, update, t, accent, pro, setTab }) {
  ...
  return (
    <div>
      ...
      {view === "recettes" ? (
        <Recettes {...{ state, update, t, accent, pro, setTab }} />
      ) : (
        <>
          {/* Sélecteur de date */}
...
function Recettes({ state, update, t, accent, pro, setTab }) {
  const [m, setM] = useState({ name: "", kcal: "", protein: "", carbs: "", fat: "" });
